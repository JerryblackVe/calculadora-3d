...
export const saveProductos = async (productos) => {
  ...
  for (const p of productos) {
    ...
    console.log('>>> saveProductos: updating id:', p.id, productoData)
    const { data, error } = await supabase
      .from('productos')
      .update(productoData)
      .eq('id', p.id)
    console.log('>>> saveProductos result for id', p.id, '- error:', error)
    if (error) console.error('Error saving producto', p.id, error)
  }
  
  console.log('>>> saveProductos: all done')
  return { error: null }
}
