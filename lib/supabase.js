...
export const saveConfig = async (config) => {
  ...
  if (!supabase) {
    console.error('>>> saveConfig: Supabase not initialized!')
...
export const loadConfig = async () => {
  ...
  if (!supabase) {
    console.error('>>> loadConfig: Supabase not initialized!')
...
export const saveProductos = async (productos) => {
  ...
  if (!supabase) {
    console.error('>>> saveProductos: Supabase not initialized!')
    return { error: 'Supabase no configurado' }
  }
...
export const loadProductos = async () => {
  ...
  if (!supabase) {
    console.error('>>> loadProductos: Supabase not initialized!')
    return { data: null, error: 'Supabase no configurado' }
  }
  const { data, error } = await supabase
    .from('productos')
